...
export default function MovableRectangles({
  ...
  const translateXValues = boxes.map((box) => useSharedValue(box.x * (scaledDimensions.width / imageDimensions.width)));
  const translateYValues = boxes.map((box) => useSharedValue(box.y * (scaledDimensions.height / imageDimensions.height)));
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {boxes.map((box, index) => {
          ...
          const scaledWidth = width * (scaledDimensions.width / imageDimensions.width);
          const scaledHeight = height * (scaledDimensions.height / imageDimensions.height);
          ...
          const drag = Gesture.Pan().onUpdate((event) => {
            ...
            translateXValues[index].value = (box.x * (scaledDimensions.width / imageDimensions.width)) + event.translationX;
            translateYValues[index].value = (box.y * (scaledDimensions.height / imageDimensions.height)) + event.translationY;
          });
