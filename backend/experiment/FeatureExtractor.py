...
class FeatureExtractor:
    ...
    def calc_NCC(self, F, M):
        ...
        NCC = np.sum(
            tmp * F.reshape(F.shape[-3], 1, 1, F.shape[-2], F.shape[-1]), axis=(0, 3, 4)
        )
        return NCC
    ...
    def __call__(self, template, image, threshold=None, use_cython=True):
        ...
        if use_cython:
            ...
            cython_calc_NCC.c_calc_NCC(
                M.flatten().astype(np.float32),
                np.array(M.shape).astype(np.int32),
                F.flatten().astype(np.float32),
                np.array(F.shape).astype(np.int32),
                self.NCC,
            )
        ...
        for max_index in max_indices:
            ...
            if i_star >= 1 and j_star >= 2:
                ...
                NCC_part[:, 1:4] = self.NCC[
                    i_star - 1 : i_star + 2, j_star - 1 : j_star + 2
                ]
            elif i_star >= 1 and j_star == 0:
                NCC_part[:, 2:4] = self.NCC[
                    i_star - 1 : i_star + 2, j_star : j_star + 2
                ]

            x_center = (
                (j_star + self.template_feature_map.size()[-1] / 2)
                * image.size()[-1]
                // self.image_feature_map.size()[-1]
            )
            y_center = (
                (i_star + self.template_feature_map.size()[-2] / 2)
                * image.size()[-2]
                // self.image_feature_map.size()[-2]
            )
            ...
            eps = 1e-12
            x1 = np.sum(
                NCC_part * (x1_0 + np.array([-2, -1, 0, 1]) * stride_product)[None, :]
            ) / (np.sum(NCC_part) + eps)
            x2 = np.sum(
                NCC_part * (x2_0 + np.array([-2, -1, 0, 1]) * stride_product)[None, :]
            ) / (np.sum(NCC_part) + eps)
            y1 = np.sum(
                NCC_part * (y1_0 + np.array([-1, 0, 1]) * stride_product)[:, None]
            ) / (np.sum(NCC_part) + eps)
            y2 = np.sum(
                NCC_part * (y2_0 + np.array([-1, 0, 1]) * stride_product)[:, None]
            ) / (np.sum(NCC_part) + eps)
            x1 = int(round(x1))
            x2 = int(round(x2))
            y1 = int(round(y1))
